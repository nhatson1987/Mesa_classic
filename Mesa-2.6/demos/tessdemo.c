...
#define TDCALLBACK CALLBACK
#else
#define TDCALLBACK CALLBACK
#endif

void TDCALLBACK my_error(GLenum err)
{
	int len,i;
	char const *str;
...
void TDCALLBACK begin_callback(GLenum mode)
{
	triangle.no=0;
}

void TDCALLBACK edge_callback(GLenum flag)
{
	if(flag==GL_TRUE)
	{
...
void TDCALLBACK vertex_callback(void *data)
{
	GLsizei no;
	GLint *p;
